...
let connectSerialFailedListeners = [];
 ...
 * 串口连接失败的回调事件
 */
function addConnectSerialFailedListener(cb) {
    if (typeof cb === "function") {
        connectSerialFailedListeners.push(cb);
    }
}
...
const init = cb => {
    ...
    socket.on("mfe-fail-connect-serial", () => {
        console.log("串口连接失败");
        connectSerialFailedListeners.forEach(cb => cb());
    });
    /**
     * 命令执行失败的通知
     */
...
const eventRegister = (
    ...
    let needParams = null;
    if (customs instanceof Array) {
        needParams = customs.join(",");
    }
    let params = `${eventTarget}, ${triggerValue}, ${delta}, ${internal}`;
    if (needParams) {
...
const eventUnregister = target => {
    let value = "Event.unregister()";
    if (target) {
        value = `Event.unregister(${target})`;
    }
    socket.emit("mfe-message", value);
};
...
module.exports = {
    ...
    addConnectSerialFailedListener,
};